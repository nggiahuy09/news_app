...
class BookmarksProvider with ChangeNotifier {
  ...
  Future<void> addToBookmark(
      {required BuildContext context, required NewsModel newsModel}) async {
    try {
      Uri uri = Uri.https(
        FIREBASE_URL,
        'bookmarks.json',
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            ...
            content: Text('Add to Bookmarks Successfully'),
          ),
        );
      }
    } catch (err) {
  ...
  Future<void> deleteFromBookmark({required BuildContext context, required String bookmarkKey}) async {
    try {
      Uri uri = Uri.https(
        FIREBASE_URL,
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            duration: Duration(seconds: 1),
            content: Text('Delete from Bookmarks Successfully'),
          ),
        );
      }
    } catch (err) {
